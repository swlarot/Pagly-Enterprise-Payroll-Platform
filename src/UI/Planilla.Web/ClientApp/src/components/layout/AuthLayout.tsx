...
export default function AuthLayout({ children }: AuthLayoutProps) {
  ...
  const getParentSection = (): string | null => {
    ...
    if (['/roles', '/audit', '/configuracion'].includes(path)) return 'Admin';
    return null;
  };

  // ── Clase de tab del top navbar (Linear-style pill) ──
  const topNavTabClass = ({ isActive }: { isActive: boolean }) =>
    `relative flex items-center gap-2 px-4 py-2 rounded-md text-[14px] font-medium
  ...
  const prominentPlanillasClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-3 py-3 rounded-lg font-semibold text-[14.5px] transition-all duration-150 border-l-2 ${
      isActive
  ...
  const subItemClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-2.5 w-full px-3 py-2 rounded-md transition-all duration-150 cursor-pointer text-[13.5px] font-medium ${
      isActive
  ...
  const adminItemClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-2.5 px-3 py-2 rounded-md transition-all duration-150 text-[13.5px] font-medium ${
      isActive
  ...
  return (
    <div className="flex h-screen bg-navy-950 flex-col">
      ...
      ══════════════════════════════════════════ */}
      <header className="h-16 sticky top-0 z-50 bg-navy-950/90 backdrop-blur-xl border-b border-white/[0.06] flex items-center flex-shrink-0">

        {/* Logo compacto */}
        <div className="flex items-center gap-2 pl-4 pr-3 flex-shrink-0">
          <PaglyLogo variant="icon" theme="dark" size="sm" />
          <span className="text-[14px] font-semibold text-white tracking-tight hidden sm:block">Pagly</span>
        </div>
        ...
        <nav className="flex items-center gap-0.5 px-3 overflow-x-auto flex-shrink-0">
          ...
          {canAccessModuleCheck('empleados') && (
            <NavLink to="/empleados" className={topNavTabClass}>
              <Users className="w-[17px] h-[17px] flex-shrink-0" />
              <span>Empleados</span>
            </NavLink>
          )}
          ...
          {canAccessModuleCheck('departamentos') && (
            <NavLink to="/departamentos" className={topNavTabClass}>
              <Building2 className="w-[17px] h-[17px] flex-shrink-0" />
          ...
          {canAccessModuleCheck('posiciones') && (
            <NavLink to="/posiciones" className={topNavTabClass}>
              <Briefcase className="w-[17px] h-[17px] flex-shrink-0" />
        ...
        <div className="flex items-center gap-1.5 pr-4 flex-shrink-0">
          ...
          {/* Notificaciones */}
          <button className="relative p-2 rounded-lg text-gray-500 hover:text-gray-300 hover:bg-white/[0.06] transition-all duration-150">
            <Bell className="w-4 h-4" />
            <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-primary-500 rounded-full" />
          </button>

          {/* Fecha */}
          <span className="text-[11px] text-gray-600 hidden md:block px-1">
          ...
          <div className="flex items-center gap-2 pl-1">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center ring-1 ring-white/10">
              <span className="text-white text-[10px] font-bold">{userInitials}</span>
            </div>
            <div className="leading-none hidden lg:block">
              <p className="text-[12px] font-semibold text-gray-100">{userName}</p>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        ══════════════════════════════════════════ */}
        <aside className="w-[260px] bg-gradient-to-b from-navy-950 via-navy-950 to-[#0a1929] border-r border-white/[0.06] flex flex-col flex-shrink-0">
          ...
          <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
            ...
            <NavLink to="/mi-perfil" className={primaryItemClass}>
              {hasRole(TenantRole.Owner) ? (
                <Users2 className="w-[19px] h-[19px] flex-shrink-0" />
              ) : (
                <User className="w-[19px] h-[19px] flex-shrink-0" />
              )}
          ...
          <div className="px-3 pt-2 pb-3 border-t border-white/[0.05]">
            ...
            {/* Tenant Card */}
            <div className="p-2 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center gap-2.5 group hover:bg-white/[0.06] transition-all duration-150">
              <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-primary-600 to-accent-600 flex items-center justify-center flex-shrink-0">
                <span className="text-white text-[10px] font-bold">{tenantInitials}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-semibold text-gray-200 truncate">{tenant?.name}</p>
                <p className="text-[10px] text-gray-600 truncate">RUC {tenant?.ruc}-{tenant?.dv}</p>
              </div>
              <button
                onClick={handleLogout}
                className="p-1 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all duration-150"
                title="Cerrar sesión"
              >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {!isPrimaryRoute && (
            <div className="h-10 px-6 flex items-center border-b border-white/[0.04] flex-shrink-0">
              {parentSection && (
                <>
                  <span className="text-sm text-gray-500">{parentSection}</span>
                  <ChevronRight className="w-3.5 h-3.5 text-gray-600 mx-1" />
                </>
              )}
              <span className="text-sm font-medium text-gray-300">{pageTitle}</span>
            </div>
          )}
