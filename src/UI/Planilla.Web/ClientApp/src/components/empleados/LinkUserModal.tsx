...
export function LinkUserModal({
  ...
  const loadSugerencias = async () => {
    try {
      ...
      const response = await fetch(`/api/empleados/${empleadoId}/sugerencias-usuarios`);

      if (!response.ok) {
        let errorMessage = 'Error al cargar usuarios disponibles';
        try {
          const error = await response.json();
          errorMessage = error.error || error.message || errorMessage;
        } catch {
          // Si no es JSON, usar mensaje genérico con código de estado
  ...
  const handleVincular = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/empleados/${empleadoId}/vincular-usuario`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        let errorMessage = 'Error al vincular usuario';
        try {
          const error = await response.json();
          errorMessage = error.error || error.message || errorMessage;
        } catch {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-gray-200 flex justify-between items-center">
          <div>
            <h3 className="text-xl font-semibold text-gray-900">Vincular Usuario</h3>
            <p className="text-sm text-gray-600 mt-1">
              Selecciona un usuario para vincular a <strong>{empleadoNombre}</strong>
            </p>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <div className="font-semibold text-gray-900">{user.nombreCompleto}</div>
                        {user.esSugerencia && (
                    ...
                    {selectedUserId === user.userId && (
                      <div className="flex items-center gap-2 text-blue-600 font-medium ml-4">
